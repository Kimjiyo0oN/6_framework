...
//-> model에 추가된 속석 중 key가 일치하는 속성을  session scope 속성으로 추가
//@SessionAttributes( {"loginMember","test"} ) 여러개를 쓸수 있다
...
public class MemberController {
	...
	public String login(HttpServletRequest req) {
		...
		// 사용자에게(웹 브라우저에서) URL이 변경되어 보이는지 여부
		// * forward 방법 : prefix/suffix를 제외한 나머지 jsp 경로
		// * redirect(재요청) 방법 : "redirect:요청주소"
		
		return "redirect:/";
	}
	
	// 2. @RequestParam 어노테이션 사용 - 요청 파라미터
	...
	// -> required = false인 경우 전달된 피라미터가 없으면 null
	...
	//@RequestMapping(value="/member/login", method = RequestMethod.POST)
	public String login(@RequestParam("inputEma il") String email,
			@RequestParam(value="inputPW1", required=true, defaultValue="1234") String pw,
			String inputPW) { //String inputPW -> @RequestParam 생략가능
		// 홈페이지에서 비번을 안적었을때는 빈칸으로 인식되서 오류가 나지 않고
		// value="inputPW"를 없는 값? 없는 변수으로 넣으면 오류 
		// required는 기본값이 true
		//@RequestParam(value="inputPW", required=true, defaultValue="1234")
		//-> 값이 null이면 1234값으로 나온다 
...
//			model.addAttribute("message", "아이디 또는 비밀번호가 일치하지 않습니다");
	 ...
	 * 
	 * 
	 * 3순위 : @ControllerAdvicec 어노테이션 
	 * 		-> 전역(웹 애플리케이션)에서 발생하는 에외를 모아서 처리
	 * 		- 별도 클래스로 작성 
	 * 
	 * 기본값으로 3순위를 깔고 가고 별도로 필요한 
	 * 
	 * 
	 * 
	 * */
...
//	@ExceptionHandler(Exception.class)
	public String exceptionHandler(Exception e, Model model) {
		...
		model.addAttribute("errorMessage", "회원 관련 서비스 이용 중 문재가 발생했습니다.");
